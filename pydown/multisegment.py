...
## {{{ http://code.activestate.com/recipes/114217/ (r1)
# mget.py
#
# by Nelson Rush
#
    ...
    def handle_read(self):
        ...
        if self.recvhead == 1:
            ...
            # If the parts is greater than 1, split into segments.
                ...
                if self.parts == 2:
                    self.h.append(http_client(
                        self.host, self.path, 0, l + 1, self.length, self.m))
                if self.parts > 2:
                    for i in range(1, self.parts - 1):
                        self.h.append(http_client(self.host, self.path,
                                      0, (i * l) + 1, (i + 1) * l, self.m))
                    self.h.append(http_client(self.host, self.path,
                                  0, ((i + 1) * l) + 1, self.length, self.m))
        ...
        # Stage 2, clip the second incoming header and start grabbing the file itself.
            ...
            if size > 0:
                ...
                if len(xrange(size / 1024)) == 0:
                    ...
                    print "Jet %lu-%lu   %lu of %lu bytes recieved" % (
...
## end of http://code.activestate.com/recipes/114217/ }}}
